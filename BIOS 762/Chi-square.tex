...
\subsection{Normal distribution and variance}
Suppose that $y_i \sim N(\mu_i, \sigma^2)$, and independent for $i=1,..n$, where $\mu_i$ and $\sigma^2$ are unknown.
\begin{itemize}
	\item [(a)] Suppose that $\mu_i = x_i^T \beta$, where $x_i$ is a $p \times 1$ vector of covariates for the ith subject, $i=1,..n$. Suppose $\beta$ is the vector parameter of interest and $\sigma^2$ is nuisance parameter. Derive the conditional likelihood of $\beta$ and a closed form estimate of the conditional MLE of $\beta$, and compare the result to the MLE of $\beta$ of full likelihood.
	
	The matrix form and projection operator generally applies to normal distribution, so we will usually write in matrix form for multivariate normal distribution. Because the likelihood function could be considered as MVN, as we are estimating $\beta, \sigma^2$ using all the $y_i$ simultaneously.
	
\begin{align*}
	p{(Y, \mu_i, \sigma^2)} &= \left( \frac{1}{\sqrt{2\pi}} \right)^n \left( \frac{1}{\sigma}\right)^n exp\{ \frac{(Y-\mu)^T (Y-\mu)}{2 \sigma^2} \}
\end{align*}	
	
We can use $M= X(X^T X)^{-1}X^T$ to get the $\beta$
...
Also we have 
\begin{align*}
	Z &= \frac{Y-\mu}{\sigma I} \sim N(0, I)\\
	\frac{Y^T (I-M)Y}{\sigma^2 I} & \sim \chi^2_{n-p}
\end{align*}
So we have distribution for $\sigma^2$

\begin{align*}
	p(z) &= \frac{1}{2^{k/2} \Gamma{k/2}} z^{k/2-1} exp(-\frac{z}{2})  \\
	x &= \sigma^2 z, \qquad \diffp{z}{x} = \sigma^{-2} \\
	p(x) &= \sigma^{-2} \frac{1}{2^{k/2} \Gamma{(k/2)} } \left(\frac{x}{\sigma^2}\right) ^{k/2-1} exp(-\frac{x}{2 \sigma^2 })
\end{align*}
The conditional log-likelihood of $\beta$ 	
\begin{align*}
	l_c &= log p(y) - log p(x) = - n log \sqrt{2\pi} -n log \sigma I + \frac{-(Y- X \beta)^T (Y- X\beta)}{2 \sigma^2} \\
	& - log \sigma I + (k/2-1) log \frac{x}{\sigma^2} - log 2^{k/2} \Gamma{(k/2)} -  \frac{x}{2 \sigma^2 }
\end{align*}
Taking first derivative, we see that the distribution of $\sigma^2$ is free of $\beta$
\begin{align*}
...
Then we also have log-likelihood 
...
\item[(b)] Suppose $\beta$ is nuisance parameter, and $\sigma^2$ is the parameter of interest. Derive the conditional distribution and calculate conditional MLE.

If we need to assume value for $\sigma$, it is the $S_n$ that includes $\sigma$, so need to separate out the situation from above question.
