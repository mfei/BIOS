...
The two common convergence mode is converges almost everywhere (a.e) and converges in measure (probability if the measurable function is probability), converges in $L_r$
...
\item[(vii)]  If $X_n \xrightarrow {d} X$, and $P(X = c)= 1$, where $c \in R^k$ is a constant vector, then $X_n \xrightarrow {p} c$.

\item[(viii)] Moment convergence: Suppose that $X_n \xrightarrow {d} X$, then for any $r > 0$, 
 \begin{align*}
   lim_{n \rightarrow \infty} E \Vert X_n \Vert_r^r &= E \Vert X \Vert_r^r < \infty
 \end{align*}
iff $\{ \Vert X_n \Vert_r^r \}$ is uniformly integrable (UI) in the sense that 
 \begin{align*}
   lim_{t \rightarrow \infty} sup E (\Vert X_n \Vert_r^r  I_{\Vert X_n \Vert_r > t})&= 0
 \end{align*}
In particular, $X_n \xrightarrow {L_r} X$ if and only if $\{ \Vert X_n - X\Vert_r^r \}$ is UI
\end{itemize}

Prove (iii) need to use the definition of convergence in probability, and construct the cumulative probability $F_X(x)$.
